...
public class MainTest {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      //open thread to take
      for(int i=0;i<200;i++){
        Thread phase2Thread=new Thread(new ConsumerPhase2Task(queue,skiersApi,sucessNumber,failNumber,countDownLatch2));
        phase2Thread.start();
      }
      countDownLatch2.await();
      long timeUsed=System.currentTimeMillis()-start;
      System.out.println("phase 2 done with "+sucessNumber +"success request "+failNumber+" fail request"
      +" spend "+String.valueOf(timeUsed));
      System.out.println("throughput "+ (int)200000/(timeUsed/1000));
